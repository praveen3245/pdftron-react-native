...
const propTypes = {
    ...
    selectCloudRectangleTool: PropTypes.bool,
...
export class DocumentView extends PureComponent {
    ...
    onChange = (event) => {
        ...
        else if (event.nativeEvent.onCompareButtonPressed) {
            if (this.props.onCompareButtonPressed) {
                this.props.onCompareButtonPressed();
            }
        }
    ...
    compareFiles = (url1, url2) => {
        console.log("jscompareFiles ::", url1);
        const tag = findNodeHandle(this._viewerRef);
        if (tag != null) {
            return DocumentViewManager.compareFiles(tag, url1, url2);
        }
        return Promise.resolve();
    };
    compareButton = (buttonState) => {
        console.log("jsButtonState ::", buttonState);
        const tag = findNodeHandle(this._viewerRef);
        if (tag != null) {
    ...
    textSearchInPdfContent = (fileUrl, searchTerm) => {
        const tag = findNodeHandle(this._viewerRef);
        if (tag != null) {
          console.log("fileUrl ::", fileUrl);
          return DocumentViewManager.textSearchInPdfContent(tag, fileUrl, searchTerm);
        }
        return Promise.resolve();
      }
      compareTwoDocument = (fileUrl1, fileUrl2) => {
        const tag = findNodeHandle(this._viewerRef);
        if (tag != null) {
          console.log("fileUrl ::", fileUrl1);
          return DocumentViewManager.compareTwoDocument(tag, fileUrl1, fileUrl2);
        }
        return Promise.resolve();
      }
    openAnnotationList = () => {
